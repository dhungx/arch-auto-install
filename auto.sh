...
```
# FIX #4: Prevent duplicate nvidia modules in mkinitcpio.conf
...
```
    # FIX #8: Verify boot entry exists
...
# FIX #5: Fallback to GRUB if systemd-boot failed
...
# FIX #5: Fatal error if no bootloader installed
if [[ $BOOTLOADER_OK -eq 0 ]]; then
error “BOOTLOADER INSTALLATION FAILED! Hệ thống sẽ KHÔNG boot được. Kiểm tra log và thử lại.”
fi

# FIX #8: Verify bootloader entry in EFI (UEFI only)
if [[ “$BOOT_MODE” == “uefi” ]]; then
...
```
# FIX #9: Check if build succeeded
if [[ -f PKGBUILD ]]; then
